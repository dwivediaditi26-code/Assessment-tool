...
export default function PostureAnalysisModule(){
  ...
  async function analysePhoto(url,v){
    ...
    return new Promise(resolve=>{
      ...
      img.onload=async()=>{
        ...
        // Draw image to offscreen canvas BEFORE sending to MediaPipe
        // (after send() the img element may be tainted/recycled in some browsers)
        ...
        const handler=results=>{
          ...
          if(results.poseLandmarks?.length>0){
            ...
            // Draw overlay on top of already-drawn image
          ...
          {scoreData&&(
            <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:16,padding: isWide?"18px":"14px",background:PC.surface,borderRadius:14,border:`1px solid ${scoreData.colour}30`,boxShadow:isWide?"0 2px 12px rgba(0,0,0,0.06)":"none"}}>
              ...
              <div style={{flex:1}}>
                ...
                <div style={{fontSize: isWide?"0.75rem":"0.68rem",color:PC.muted,marginTop:3}}>
                ...
                {measurements?._calibrated&&(
                  <div style={{marginTop:8,display:"flex",flexWrap:"wrap",gap:5}}>
                    ...
                    {measurements.trunkShiftCm!=null&&measurements.trunkShiftCm>0.5&&(
                      <span style={{padding:"2px 8px",borderRadius:6,fontSize:"0.62rem",fontWeight:700,
                        background:"rgba(180,83,9,0.1)",color:PC.yellow,
                        border:`1px solid ${PC.yellow}40`}}>
                        Trunk shift {measurements.trunkShiftCm}cm
                      </span>
                    )}
                  </div>
                )}
  ...
  const leftPanel = (
    <div style={{display:"flex",flexDirection:"column",flex: isWide?"0 0 480px":"1",minWidth:0,borderRight: isWide?`1px solid ${PC.border}`:"none"}}>
        ...
        {isLive?(
          <div>
            {!camReady?(
              ...
              <div>
                <div style={{position:"relative",background:"#0a0a14",aspectRatio:"4/3",overflow:"hidden"}}>
                  ...
                  {scoreData&&<div style={{position:"absolute",top:8,right:8}}><ScoreRing score={scoreData.score} band={scoreData.band} colour={scoreData.colour} size={isWide?84:70}/></div>}
                  {countdown!==null&&(
          ...
          <div style={{padding: isWide?"20px":"16px"}}>
            ...
            {inputMode==="ai"&&uploadedImg&&(
              <div style={{borderRadius:14,overflow:"hidden",border:`1px solid ${PC.border}`,boxShadow:isWide?"0 4px 20px rgba(0,0,0,0.08)":"none"}}>
  ...
  return(
    <div style={{background:PC.bg,minHeight:"100vh",fontFamily:"system-ui,-apple-system,sans-serif",display:"flex",flexDirection:"column"}}>
      ...
      {/* ── Body ── */}
      <div style={{flex:1,display:"flex",flexDirection:isWide?"row":"column",overflow:isWide?"hidden":"visible",minHeight:isWide?"calc(100vh - 60px)":0}}>
        {leftPanel}
        {isWide?(
