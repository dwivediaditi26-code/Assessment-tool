...
export default function PostureAnalysisModule(){
  ...
  function generateReport() {
    ...
    // Open print window with embedded HTML
    ...
    win.document.write(`<!DOCTYPE html><html><head>
      <meta charset="utf-8"/>
      <title>PostureAI ${reportType==="basic"?"Basic":"Detailed"} Report — ${d.patient.name}</title>
      ...
      <style>
        *{box-sizing:border-box;margin:0;padding:0;}
        body{font-family:'DM Sans',sans-serif;background:#e2e8f0;color:#1a1025;}
        @media print{body{background:#fff;}.noprint{display:none!important;}.page{box-shadow:none!important;margin:0!important;border-radius:0!important;page-break-after:always;}.page:last-of-type{page-break-after:auto;}}
        .page{width:794px;min-height:1123px;background:#fff;margin:0 auto 24px;border-radius:4px;box-shadow:0 4px 32px rgba(0,0,0,.12);position:relative;overflow:hidden;}
        .noprint{position:sticky;top:0;z-index:100;background:#0f172a;padding:12px 24px;display:flex;justify-content:space-between;align-items:center;}
        table{border-collapse:collapse;width:100%;}
        th,td{text-align:left;}
      </style>
    </head><body>
      <div class="noprint">
        <span style="color:#fff;font-family:Fraunces;font-weight:900;font-size:1.1rem;">PostureAI · ${reportType==="basic"?"Basic (2 credits)":"Detailed (5 credits)"} Report</span>
        <button onclick="window.print()" style="padding:8px 20px;border:none;border-radius:8px;background:#0ea5e9;color:#fff;font-weight:700;font-size:0.8rem;cursor:pointer;">🖨 Print / Save PDF</button>
      </div>
      <div id="report-root" style="padding:24px 0;max-width:860px;margin:0 auto;"></div>
      <script>
        window.__REPORT_DATA__ = ${JSON.stringify(d)};
        window.__REPORT_TYPE__ = "${reportType}";
        window.__CREDITS__ = ${credits};
      </script>
    </body></html>`);

    // Inject the actual React-rendered HTML via innerHTML after fonts load
    // Since we can't bundle React here, render as static HTML directly
    win.document.close();
    setTimeout(()=>{
      const root = win.document.getElementById("report-root");
      if(root) root.innerHTML = buildStaticReport(d, reportType, credits);
    }, 800);
