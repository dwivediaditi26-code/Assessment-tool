...
function drawOverlay({ctx,W,H,lm,view,showGrid,measurements}) {
  ...
  // ── FRONTAL-ONLY Feature 3 overlays ──────────────────────────────────────
  if(!isLat){

    // 1. Head tilt line between ears with angle badge
    if(V(7)&&V(8)){
      ...
      ctx.save();
      ctx.strokeStyle=tiltColor; ctx.lineWidth=2.5; ctx.setLineDash([]);
    ...
    // 2. Horizontal level lines: eyes, ears, shoulders, ASIS, knees, ankles
    const LEVELS=[
    ...
    LEVELS.forEach(({idxL,idxR,label,color})=>{
      ...
      const extend=W*0.06;
      drawLevelLine(ctx, pL[0]-extend, pL[1], pR[0]+extend, pR[1], color, label);
    });

    // 3. Waist triangle shading L vs R
    if(V(11)&&V(13)&&V(23)&&V(12)&&V(14)&&V(24)){
      const pSL=PX(11),pEL=PX(13),pHL=PX(23);
      const pSR=PX(12),pER=PX(14),pHR=PX(24);
      const asymm=m.waistTriangleAsymmetry||0;
      const colorL=asymm>6?"rgba(255,77,109,0.18)":asymm>3?"rgba(255,179,0,0.15)":"rgba(0,229,160,0.12)";
      const colorR=asymm>6?"rgba(255,77,109,0.18)":asymm>3?"rgba(255,179,0,0.15)":"rgba(0,229,160,0.12)";
      const strokeL=asymm>6?"rgba(255,77,109,0.5)":asymm>3?"rgba(255,179,0,0.4)":"rgba(0,229,160,0.35)";
      // Left triangle
      ctx.beginPath(); ctx.moveTo(pSL[0],pSL[1]); ctx.lineTo(pEL[0],pEL[1]); ctx.lineTo(pHL[0],pHL[1]); ctx.closePath();
      ctx.fillStyle=colorL; ctx.fill();
      ctx.strokeStyle=strokeL; ctx.lineWidth=1.5; ctx.setLineDash([4,3]); ctx.stroke(); ctx.setLineDash([]);
      // Right triangle
      ctx.beginPath(); ctx.moveTo(pSR[0],pSR[1]); ctx.lineTo(pER[0],pER[1]); ctx.lineTo(pHR[0],pHR[1]); ctx.closePath();
      ctx.fillStyle=colorR; ctx.fill();
      ctx.strokeStyle=strokeL; ctx.lineWidth=1.5; ctx.setLineDash([4,3]); ctx.stroke(); ctx.setLineDash([]);
      // Asymmetry badge
      if(asymm>3){
        const bx=(pSL[0]+pSR[0])/2, by=Math.min(pEL[1],pER[1])-22;
        const bColor=asymm>6?"#ff4d6d":"#ffb300";
        drawBadge(ctx, bx, by, `Waist ${asymm.toFixed(1)}%`, bColor);
      }
    }

    // 4. LLD vertical arrow between ankle heights
    if(V(27)&&V(28)){
      const pL=PX(27), pR=PX(28);
      const lldMm=m.ankleLLDmm;
      if(lldMm!==null&&lldMm>3){
        const higher=pL[1]<pR[1]?pL:pR; // lower y = higher up = shorter limb
        const lower=pL[1]<pR[1]?pR:pL;
        const arrowX=Math.max(pL[0],pR[0])+W*0.04;
        const arrowColor=lldMm>10?"#ff4d6d":lldMm>5?"#ffb300":"#aaa";
        ctx.save();
        ctx.strokeStyle=arrowColor; ctx.lineWidth=2; ctx.setLineDash([]);
        // Vertical line
        ctx.beginPath(); ctx.moveTo(arrowX,higher[1]); ctx.lineTo(arrowX,lower[1]); ctx.stroke();
        // Arrow heads
        const ah=7;
        ctx.beginPath(); ctx.moveTo(arrowX,higher[1]); ctx.lineTo(arrowX-ah/2,higher[1]+ah); ctx.lineTo(arrowX+ah/2,higher[1]+ah); ctx.closePath(); ctx.fillStyle=arrowColor; ctx.fill();
        ctx.beginPath(); ctx.moveTo(arrowX,lower[1]); ctx.lineTo(arrowX-ah/2,lower[1]-ah); ctx.lineTo(arrowX+ah/2,lower[1]-ah); ctx.closePath(); ctx.fill();
        // Horizontal connectors
        ctx.setLineDash([4,3]);
        ctx.beginPath(); ctx.moveTo(higher[0],higher[1]); ctx.lineTo(arrowX,higher[1]); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(lower[0],lower[1]); ctx.lineTo(arrowX,lower[1]); ctx.stroke();
        ctx.restore(); ctx.setLineDash([]);
        // Badge
        const midY=(higher[1]+lower[1])/2;
        drawBadge(ctx, arrowX+28, midY, `LLD ~${lldMm.toFixed(0)}mm`, arrowColor);
      }
    }

    // 5. Knee/ankle valgus-varus arc label
    if(V(25)&&V(26)&&V(27)&&V(28)){
      ...
      if(ratio!==null&&pattern!=="Normal"){
        ...
        const arcColor=isValgus?"rgba(249,115,22,0.85)":"rgba(99,102,241,0.85)";
        const kMx=(pKL[0]+pKR[0])/2, kMy=(pKL[1]+pKR[1])/2;
        const aMx=(pAL[0]+pAR[0])/2, aMy=(pAL[1]+pAR[1])/2;
        const midY=(kMy+aMy)/2;
        // Draw curved arc between knee and ankle midpoints
        ctx.save();
        ctx.strokeStyle=arcColor; ctx.lineWidth=2.5; ctx.setLineDash([5,4]);
        ctx.beginPath();
        ctx.moveTo(kMx,kMy);
        ctx.quadraticCurveTo(isValgus?kMx-30:kMx+30, midY, aMx, aMy);
        ctx.stroke();
        ctx.restore(); ctx.setLineDash([]);
        drawBadge(ctx, isValgus?kMx-44:kMx+44, midY, pattern, arcColor);
      }
    }
  }
}
...
// ─── View config ──────────────────────────────────────────────────────────────
const VIEWS={
  anterior:{label:"Frontal",short:"Frontal",badge:"+ Frontal plumb",colour:PC.accent,icon:"⬆",helper:"Patient faces camera, feet hip-width, arms relaxed."},
  posterior:{label:"Back",short:"Back",badge:"+ Frontal plumb",colour:PC.a2,icon:"⬇",helper:"Patient faces away. Scapulae and heels visible."},
  left:{label:"Sagittal L",short:"Sag L",badge:"+ Sagittal plumb",colour:PC.yellow,icon:"◀",helper:"Left side toward camera. Ear–shoulder–hip–ankle in frame."},
  right:{label:"Sagittal R",short:"Sag R",badge:"+ Sagittal plumb",colour:PC.green,icon:"▶",helper:"Right side toward camera. Ear–shoulder–hip–ankle in frame."},
};
